...
`Filtering` allows you to search data based on criteria by range. Search uses `greaterOrEqualThan`(>=) and `lessOrEqualThan`(<=) operators. Data format is YYYY-MM-DD HH:MM:SS. When the date is first set in the filter field, the time values will either be 00:00:00 or 23:59:59, depending on the date field.
### How does it look?
=== "List widget"
